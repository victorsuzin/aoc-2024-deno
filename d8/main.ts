  ...
  for(const frequency of frequencies){
    ...
    for(let i = 0; i < cur_antennas.length - 1; i++){
      ...
      for(let j = i + 1; j < cur_antennas.length; j++){
        ...
        const antinode_1 = {"row": cur_antenna.row, "col": cur_antenna.col};
        while(
          antinode_1.row >= 0 &&
          antinode_1.col >= 0 &&
          antinode_1.row < splitted_rows.length &&
          ...
          if(!antinodes_locations.filter(e => e.row === antinode_1.row && e.col === antinode_1.col).length)
            antinodes_locations.push(JSON.parse(JSON.stringify(antinode_1)));
          antinode_1.row -= delta_row;
          antinode_1.col -= delta_col;
        }

        const antinode_2 = {"row": next_antenna.row, "col": next_antenna.col};
        while(
          antinode_2.row >= 0 &&
          antinode_2.col >= 0 &&
          antinode_2.row < splitted_rows.length &&
          ...
          if(!antinodes_locations.filter(e => e.row === antinode_2.row && e.col === antinode_2.col).length)
            antinodes_locations.push(JSON.parse(JSON.stringify(antinode_2)));
          antinode_2.row += delta_row;
          antinode_2.col += delta_col;
        }
      }
    }
  }
